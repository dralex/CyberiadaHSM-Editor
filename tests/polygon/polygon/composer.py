...
def corpus(env):
    """[(name, path)] of the corpus documents and the empty start document."""
    ...
    return out, folder / manifest["start"]
...
def compose(kind, env, catalog, coverage, seed, name=None):
    rng = random.Random(seed)
    diagrams, empty = corpus(env)
    if name is None:
        name, path = rng.choice(diagrams)
    else:
        path = dict(diagrams).get(name) or (env.diagrams / (name + ".graphml"))
    if kind == REPRODUCE:
        return Mission(kind, seed, name, empty, original=path)
    verbs = catalog.verbs(CAT.FORM_MODEL, CAT.FORM_GESTURE_FORM)
