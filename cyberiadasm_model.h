...
public:
    ...
    // the transition label box (position + size); an invalid rect resets it to auto-placement
    bool                                updateLabel(const QModelIndex& index, const Cyberiada::Rect& label_rect);
    bool                                setColor(const QModelIndex& index, const QString& color);
